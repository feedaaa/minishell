...
/*   Updated: 2024/08/25 12:17:33 by ffidha           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	redirect_input_until(t_statement *node)
{
	...
	while (1)
	{
		...
		if (streq(buff, node->next->argv[0]))
			// any of the buff is similar to the next node it breaks
			break ;
		ft_putendl_fd(buff, fd[1]);
			// writes the input stored in buff inside fd1
	}
	close(fd[1]);
...
static void	redirect_input(t_statement *node)
{
	...
	if (node->next->argv[0]) // the first argument of the next node
	{
		while (node->next->operator== RDR_INPUT)   
			// in case there is more than one redirector
			node = node->next;                     
				// it reaches to the last redirector command
		if (access(node->next->argv[0], F_OK) == 0) // checks if the file exists
		{
			in_file = open(node->next->argv[0], O_RDONLY, 0666);
			dup2(in_file, STDIN_FILENO);
				// makes the file descriptor the standard input
		}
...
{
	close(STDOUT_FILENO);
		// close the standard output file so than the next opened file will take its job
		...
		|| node->next->operator== RDR_OUT_APPEND)
	{
		if (node->operator== RDR_OUT_REPLACE)                              //">"
			open(node->next->argv[0], O_WRONLY | O_TRUNC | O_CREAT, 0666);
				// so the standart output will appear in this file replacing its content
