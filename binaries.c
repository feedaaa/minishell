...
void	cmd_binaries(t_statement *statement, t_data *data)
{
	...
	paths = get_paths(data->envp_lst);
		// checks where exactly the path is located
	...
	bin_path = get_bin_path(statement->argv[0], paths);
		// returns the binary path if its not an absloute path
...
char	**get_paths(t_vlst *envp_lst)
{
	...
	while (!streq("PATH", temp->var_name))
		// searches for the line where it has PATH name in the env
	{
		temp = temp->next;
		if (temp == NULL)
			return (NULL);
	...
	return (ft_split(temp->var_value, ':'));
		// it split the paths that are seperated by a :
}
...
char	*get_bin_path(char *cmd, char **paths)
{
	...
	while (paths[i])
	{
		...
		bin_path = join_free(tmp, cmd);        
			// it joins the path with the command and searches for it
		...
		free(bin_path);                        
			// if was not found free & check the next path
		i += 1;
	}
	return (NULL);
}
