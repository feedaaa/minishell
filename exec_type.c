...
void	exec_type(t_statement *statement_list, t_data *data)
{
	...
	if (p_lstsize(statement_list) == 1)  //if it contains only one argumment of command
	{
		if (!builtin(statement_list, data) && fork() == 0) //check if its not a builtin command, the child goes inside the loop
		{
			signal(SIGINT, child_signals);  //changes depend on the child signals
			exec_executables(statement_list, data);
		}
	}
	else if (fork() == 0) //is it was more than one command
		exec_cmd(statement_list, data);
	waitpid(-1, &temp_status, 0);  //temp_status stores the status of the child process
	if (!WTERMSIG(temp_status))  //checks if the child process was not terminated by a signal
		g_exit_status = temp_status >> 8; //the 8th bit and afterwards contain the exit status of the child process
}
